...
static sk_sp<sktxt::TestFontCollection> fontCollection = nullptr;

static void initialize() {
  if (fontCollection == nullptr) {
    fontCollection = sk_make_sp<sktxt::TestFontCollection>(txt::GetFontDir());
  }
  if (canvas == nullptr) {
...
static void BM_SkParagraphShortLayout(benchmark::State& state) {
  initialize();
  ...
  auto builder = sktxt::ParagraphBuilder::make(paragraph_style, fontCollection);
...
BENCHMARK(BM_SkParagraphShortLayout);

static void BM_SkParagraphLongLayout(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphLongLayout);

static void BM_SkParagraphJustifyLayout(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphJustifyLayout);

static void BM_SkParagraphManyStylesLayout(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphManyStylesLayout);

static void BM_SkParagraphTextBigO(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphTextBigO)
    ->RangeMultiplier(4)
    ->Range(1 << 6, 1 << 14)
    ->Complexity(benchmark::oN);

static void BM_SkParagraphStylesBigO(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphStylesBigO)
    ->RangeMultiplier(4)
    ->Range(1 << 3, 1 << 12)
    ->Complexity(benchmark::oN);

static void BM_SkParagraphPaintSimple(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphPaintSimple);

static void BM_SkParagraphPaintLarge(benchmark::State& state) {
  initialize();
...
BENCHMARK(BM_SkParagraphPaintLarge);

static void BM_SkParagraphPaintDecoration(benchmark::State& state) {
  initialize();
  ...
  text_style.setDecoration(static_cast<sktxt::TextDecoration>
                            ( sktxt::TextDecoration::kLineThrough
                            | sktxt::TextDecoration::kOverline
...
BENCHMARK(BM_SkParagraphPaintDecoration);
...
// (not applicable for SkParagraph)
//
// -----------------------------------------------------------------------------
